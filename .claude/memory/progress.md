- **Popup Web-App Sync Piece 2 — Popup UI complete (2026-07-10):** Rewrote `extension/lib/popup.ts` to read `ff_display_state` from `browser.storage.local` on load and via `storage.local.onChanged`, render `projectName`, `subPieceName`, `usedSeconds`, and `totalSeconds` as a single used/total minutes card, and removed all `ff_active_session_v2` reconstruction logic; restructured `extension/entrypoints/popup.html` to the used/total card layout while keeping the pastel nature theme and Burmese-first labels; updated `extension/lib/__tests__/popup.test.ts` with `DisplayState`-based tests (20/20); `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/popup.test.ts`, and `npm run build:ext` pass. Ready for live browser verification and merge.

- **Popup Web-App Sync Piece 3 — Mode-aware + event-driven sync + completion (2026-07-11):** Added `isCompleted` to `DisplayState` in `types.ts`; rewrote `buildDisplayState` in `focusSync.ts` to detect sub-piece vs project mode and compute `usedSeconds`/`totalSeconds`/`isCompleted` accordingly; added `ff:display-sync` event listener in `startFocusSyncPolling` for instant popup updates; added `notifyDisplaySyncNeeded()` in `useTimer.ts` dispatching after every `persistSession` and before session removal on completion; updated `popup.ts` for "Completed" rendering with green dot + Burmese label; added `.status-dot.completed` CSS; updated tests in `focusSync.test.ts`, `popup.test.ts`, `controlMessage.test.ts`; `npx tsc --noEmit`, 53/53 tests, `npm run build:ext` all pass.

- **Session end — Notification Engine Synchronization v2 Pieces 1–4 complete (2026-07-09):** Pieces 1–4 implemented and verified; Piece 5 (Complete at 100% + restore on startup) and Piece 6 (acceptance scenario + re-focus tests) remain; Agent tool triggers `reasoning_effort` API error in this chat, so remaining work should be done directly without sub-agents; resume from `.claude/memory/notification-v2-piece5-resume.md`.

- **Notification Engine Synchronization v2 Piece 4 — Alarm validation + no-fire rules complete (2026-07-09):** Added `onFocusAlarm(browser, alarmName)` to `notificationEngine.ts` with parser for `focus-{sessionId}-{type}-{targetElapsed}` names; validates session exists, `sessionId` matches, and timer is running before firing; fires milestone/almost/complete notifications via `notifyFromPayload` and updates trackers; persists session after fire; routed `focus-*` alarms in `background.ts` to `onFocusAlarm`; replaced `onStageAlarm` with no-op stub in `timerEngine.ts`; added 6 validation tests to `notificationEngine.test.ts` (valid milestone/almost/complete, old sessionId ignored, paused-state ignored, duplicate ignored); updated `background.test.ts` routing assertion; fixed duplicate import and `unknown` type issues in test file; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/notificationEngine.test.ts` (26/26), `extension/lib/__tests__/background.test.ts` (9/9), and `extension/lib/__tests__/timerEngine.test.ts` (17/17) pass. Ready for Piece 5.

- **Notification Engine Synchronization v2 Piece 3 — Resume recalculation hardening complete (2026-07-09):** Added defensive `cancelNotifications(getBrowser())` call in `timerEngine.resumeSession` before rescheduling to clear any stale leftover alarms; added 4 drift-free acceptance tests to `timerEngine.test.ts` (5-hour pause, 1000 pause/resume cycles with `targetTimeSeconds: 3600` so a next target exists after 1000s active elapsed, browser sleep simulation, milestone-passed-while-paused skipped); added focused `scheduleNotifications` elapsed=100 test to `notificationEngine.test.ts`; updated `integration.test.ts` pause/resume test to assert all `focus-*` alarms are cleared, resumed alarms use `Date.now() + remaining`, and already-fired milestones are not duplicated; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts` (22/22), `npx vitest run extension/lib/__tests__/notificationEngine.test.ts` (20/20), and `npx vitest run extension/lib/__tests__/integration.test.ts` (4/4) pass. Ready for Piece 4.

- **Notification Engine Synchronization v2 Piece 2 — Alarm scheduler ownership + re-focus duration fix complete (2026-07-09):** Moved alarm scheduling from `timerEngine.ts` to `notificationEngine.ts` via new `scheduleNotifications(browser, session, elapsed)` and `cancelNotifications(browser)`; added `getSessionDuration(token)` to compute effective duration (`subPieceRemainingBaseline ?? targetTimeSeconds` for sub-piece, `max(0, targetTimeSeconds - projectElapsedBaseline)` for project) and updated `buildNotificationSchedule` to use it; alarm names now encode `focus-{sessionId}-{type}-{targetElapsed}` (with milestone index); timerEngine calls scheduler after start/resume and cancels on pause/reset; `cancelNotifications` collects names before clearing to avoid fake-browser iterator skip; updated `notificationEngine.test.ts` (11 new scheduler/duration tests), `timerEngine.test.ts` (new alarm naming/time assertions), and `integration.test.ts` (sessionId-aware alarm assertions and `Date.now() + remaining` resume times); `npx tsc --noEmit`, targeted tests `notificationEngine.test.ts` (19/19) and `timerEngine.test.ts` (18/18), plus full `extension/lib/__tests__` suite (198/198) pass. Ready for Piece 3.

- **Notification Engine Synchronization v2 Piece 1 — Session ID plumbing complete (2026-07-09):** Added `sessionId: string` to `ActiveSessionToken`; `timerEngine` generates fresh 8-char hex `sessionId` on every real `startSession`, preserves it across pause/resume/update, and clears it on reset; `messageHandler.isValidToken` now requires a non-empty `sessionId`; updated `timerEngine.test.ts` (2 new assertions) and `controlMessage.test.ts`; also fixed `sessionId` in dependent test tokens and `hooks/useTimer.ts` buildActiveSessionToken to satisfy the required type; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts` (18/18), and `npx vitest run extension/lib/__tests__/controlMessage.test.ts` (13/13) pass. Ready for Piece 2.

- **Notification Engine Rewrite Piece E complete (2026-07-09):** Removed legacy `notifySessionComplete` from `extension/lib/notifications.ts` and its tests from `extension/lib/__tests__/notifications.test.ts`; all legacy focus-session notification functions now removed; `npx tsc --noEmit` and targeted vitest run (`stageScheduler`, `notificationEngine`, `notifications`, `timerEngine`, `integration`) — 53/53 pass. Notification engine rewrite complete.

- **Notification Engine Rewrite Piece D complete (2026-07-09):** Removed legacy `notifyAlmostDone` from `extension/lib/notifications.ts` and its tests from `extension/lib/__tests__/notifications.test.ts`; `npx tsc --noEmit` and `npx vitest run extension/lib/__tests__/notifications.test.ts` (21/21) pass. Ready for Piece E (remove legacy `notifySessionComplete`).

- **Notification Engine Rewrite Piece C complete (2026-07-09):** Removed legacy `notifyStart` from `extension/lib/notifications.ts` and its tests from `extension/lib/__tests__/notifications.test.ts`; `npx tsc --noEmit` and `npx vitest run extension/lib/__tests__/notifications.test.ts` (27/27) pass. Ready for Piece D (remove legacy `notifyAlmostDone`).

- **Notification Engine Rewrite Piece B complete (2026-07-09):** Removed legacy `notifyMilestone` from `extension/lib/notifications.ts` and all related tests from `extension/lib/__tests__/notifications.test.ts`; `notifyFromPayload` remains the single firing path; `npx tsc --noEmit` and `npx vitest run extension/lib/__tests__/notifications.test.ts` (32/32) pass. Ready for Piece C (remove legacy `notifyStart`).

- **Notification Engine Rewrite Piece A complete (2026-07-09):** `extension/lib/stageScheduler.ts` rewritten to the Universal Dynamic Notification System Algorithm — `getMilestoneCount` now uses `N = max(1, floor(0.75 × log2(T/60 + 1)) + 1)` capped at 20, and milestone positions follow `gap = 50/(N+1)`, `position(i) = 25 + gap × i`, `milestoneTime(i) = T × position(i)/100`; `extension/lib/__tests__/stageScheduler.test.ts` updated for 3600s (5 milestones) and 18000s (7 milestones); `npx tsc --noEmit` and `npx vitest run extension/lib/__tests__/stageScheduler.test.ts` (9/9) pass. Ready for Piece B (remove legacy `notifyMilestone`).

- **Notification engine rewrite — context limit reached, resume next chat (2026-07-09):** User provided `Universal_Dynamic_Notification_System_Algorithm.md` and demanded the entire extension notification engine be rewritten to follow it exactly. Core violations identified: `extension/lib/stageScheduler.ts` uses a hard-coded milestone count table instead of `N = max(1, floor(0.75 × log2(T/60 + 1)) + 1)` capped at 20, and `extension/lib/notifications.ts` still keeps legacy hard-coded `notifyStart`/`notifyMilestone`/`notifyAlmostDone`/`notifySessionComplete` functions. Pause/resume/reset syncing errors explicitly deferred. Resume from `.claude/memory/notification-engine-rewrite-resume.md`.

- **Phase 8 live verification — bugfix: restore content-script event bridge (2026-07-09):** Live test found popup showed "No active session" while web app timer was running. Root cause: web app pages do not have `window.browser`, so `useTimer.ts` direct `runtime.sendMessage` calls were silently skipped after the `ff:command`/`ff:request` bridge was removed in Phase 6. Fixed by restoring the bridge in `extension/entrypoints/control.content.ts` and updating `hooks/useTimer.ts` to fall back to `ff:command`/`ff:request` events when `window.browser` is unavailable; added control.content tests for the bridge; `control.content.test.ts` (4/4), `useTimer.test.tsx` (49/49), `TimerPanel.test.tsx` (30/30), `integration.test.ts` (4/4), `npx tsc --noEmit`, `npm run build`, and `npm run build:ext` all pass. Extension needs reload and live re-test.

- **Session end — Notification engine refactor Phases 1–7 complete (2026-07-09):** All refactor work finished through integration tests. Phase 8 (live browser verification with real 1/2/5-minute sessions) remains for next session. Uncommitted changes include modified extension timer/message/popup/focusSync code, updated web-app useTimer/TimerPanel, new integration tests, and memory/skill updates. Resume from Phase 8.

- **Notification Engine Refactor Phase 7 — Integration tests complete (2026-07-09):** Created `extension/lib/__tests__/integration.test.ts` with 4 end-to-end tests: 1-minute session (Start + 1 milestone + Almost + Complete), 5-minute session (Start + 2 milestones + Almost + Complete), pause/resume (clears/recreates alarms, no duplicates), reset (fresh start); uses `fakeBrowser` for alarms/notifications/storage and mocked `Date.now` for deterministic timing; `npx tsc --noEmit`, `integration.test.ts` (4/4), and `npm run build:ext` all pass. Ready for Phase 8 (Live browser verification).

- **Notification Engine Refactor Phase 6 — Old sync remnants removed (2026-07-09):** `extension/entrypoints/control.content.ts` now only forwards `STATE_UPDATED` → `ff:state`; removed old `EXT_START_TIMER`/`EXT_PAUSE_TIMER`/`EXT_RESET_TIMER` forwarding and `ff:command`/`ff:request` listeners; `ExtensionTimerState` cleaned up to remove legacy tracker fields (`startFired`, `lastMilestone`, `almostDoneFired`, `completeFired`); `hooks/useTimer.ts` documented `ff_active_session` as web-app restore only; `control.content.test.ts` reduced to 2 focused tests; `npx tsc --noEmit`, `control.content.test.ts` (2/2), `useTimer.test.tsx` (49/49), `TimerPanel.test.tsx` (30/30), `npm run build`, and `npm run build:ext` all pass. Ready for Phase 7 (Integration tests).

- **Notification Engine Refactor Phase 5c — Popup + focusSync cleanup complete (2026-07-09):** `extension/lib/popup.ts` now queries `GET_ACTIVE_SESSION`, derives display state from `ActiveSessionToken` when present and falls back to stored `ExtensionTimerState`, and sends `RESUME_SESSION`/`PAUSE_SESSION`/`RESET_SESSION`; Start is disabled when no active token exists; old `START_TIMER`/`PAUSE_TIMER`/`RESET_TIMER` payload path removed; `extension/lib/focusSync.ts` removed `syncFocusSession`/`UPDATE_SCHEDULE_INPUTS` forwarding entirely (only settings sync remains); updated `popup.test.ts` (19/19), `popup-start.test.ts` (6/6), `popup-pause-reset.test.ts` (7/7), and `focusSync.test.ts` (7/7); `npx tsc --noEmit`, all four test runs, and `npm run build:ext` pass. Ready for Phase 6.

- **Notification Engine Refactor Phase 5b — TimerPanel wiring complete (2026-07-09):** `components/timer/TimerPanel.tsx` extension control listeners (`ff:start`, `ff:pause`, `ff:reset`) now register whenever an active project exists, including project-only focus mode; `ff:state` handler remains compatible with the `ExtensionTimerState` shape; `components/timer/__tests__/TimerPanel.test.tsx` added 3 extension-event tests asserting `start`, `pause`, and `reset` are called on the dispatched events; `npx tsc --noEmit`, `npx vitest run components/timer/__tests__/TimerPanel.test.tsx` (30/30), `npx vitest run components/timer/__tests__/TimerPanel.session-summary.test.tsx` (9/9), and `npm run build` all pass. Ready for Phase 5c (Popup + focusSync cleanup).

- **Notification Engine Refactor Phase 5a — useTimer sends new extension commands (2026-07-08):** `hooks/useTimer.ts` now builds `ActiveSessionToken`, sends `START_SESSION`/`RESUME_SESSION`/`PAUSE_SESSION`/`RESET_SESSION` via `window.browser.runtime.sendMessage`, and seeds display from `GET_ACTIVE_SESSION`; post-agent bugfix corrected message field from `command` to `type` and replaced old `GET_TIMER_STATE`/`ff:request` path with direct `sendMessage` to `GET_ACTIVE_SESSION`; `hooks/__tests__/useTimer.test.tsx` updated (49/49 passing); `npx tsc --noEmit`, `npm run build`, and `npm run build:ext` all pass. Ready for Phase 5b (TimerPanel wiring).

- **Notification Engine Refactor Cleanup B complete (2026-07-08):** `notifyFromPayload` added to `extension/lib/notifications.ts`; `timerEngine` fires notifications via payload-based IDs; `npx tsc --noEmit`, notifications test (42/42), timerEngine test (16/16), and `npm run build:ext` all pass. Ready for Phase 5.

- **Notification Engine Refactor Phase 4b — Background alarm routing complete (2026-07-08):** Updated `extension/entrypoints/background.ts` to inject browser/handler dependencies via `setBackgroundBrowserInstance`, `setBackgroundMessageHandler`, `setBackgroundStageAlarmHandler`, and `setBackgroundRestoreOnStartup`; the `defineBackground` main registers `browser.runtime.onMessage` -> `handleMessage`, `browser.alarms.onAlarm` -> `timerEngine.onStageAlarm` for `focus-*` alarms, and calls `timerEngine.restoreOnStartup()` on init; removed old `focus-timer` alarm branch; updated `extension/lib/__tests__/background.test.ts` (9 tests); `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/background.test.ts`, and `npm run build:ext` all pass. Ready for Phase 5 (Web app integration).

- **Notification Engine Refactor Phase 4a — Message Handler commands complete (2026-07-08):** Rewrote `extension/lib/messageHandler.ts` to the new `TimerMessage` command surface (`SET_ACTIVE_SESSION`, `START_SESSION`, `RESUME_SESSION`, `PAUSE_SESSION`, `RESET_SESSION`, `UPDATE_SESSION`, `GET_ACTIVE_SESSION`), routing each to the matching `timerEngine` function and returning `{ ok: true }`/`{ ok: false, error }`; old `START_TIMER`/`PAUSE_TIMER`/`RESET_TIMER`/`UPDATE_TIMER_STATE` branches removed; updated `extension/lib/__tests__/controlMessage.test.ts` (13 tests) and `extension/lib/__tests__/background.test.ts` (8 tests) for the new types; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/controlMessage.test.ts`, and `npm run build:ext` all pass. Ready for Phase 4b (Background alarm routing).

- **Notification Engine Refactor Phase 3a — Timing Engine core complete (2026-07-08):** Rewrote `extension/lib/timerEngine.ts` to own `ActiveSessionToken`, `SessionTrackers`, and `sessionElapsed` calculation; added `startSession`, `resumeSession`, `pauseSession`, `resetSession`, `updateSession`, `getSessionElapsed`, `getActiveSession`, `restoreOnStartup`; updated `extension/lib/sessionStorage.ts` to store `StoredSession` (token + trackers) under `ff_active_session_v2`; rewrote `extension/lib/__tests__/timerEngine.test.ts` (7 tests); `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts`, and `npm run build:ext` all pass. Ready for Phase 3b (alarms + notification firing).

- **Notification Engine Refactor Phase 2 — Pure Notification Engine complete (2026-07-08):** Created `extension/lib/notificationEngine.ts` with `buildNotificationSchedule`, `prepareStartPayload`, `prepareMilestonePayload`, `prepareAlmostPayload`, `prepareCompletePayload`; defined `NotificationSchedule` and `NotificationPayload` types; wraps `buildStageSchedule` from `extension/lib/stageScheduler.ts`; created `extension/lib/__tests__/notificationEngine.test.ts` (8 tests); `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/notificationEngine.test.ts`, and `npm run build:ext` all pass. Ready for Phase 3 (Timing Engine).

- **Notification Engine Refactor Phase 1 — Active Session Token + storage wrapper complete (2026-07-08):** Added `ActiveSessionToken` interface to `extension/lib/types.ts`; created `extension/lib/sessionStorage.ts` with `getActiveSession`, `setActiveSession`, `clearActiveSession` using key `ff_active_session_v2` and a swappable browser instance for tests; created `extension/lib/__tests__/sessionStorage.test.ts` (3 tests); `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/sessionStorage.test.ts`, and `npm run build:ext` all pass. Ready for Phase 2 (Notification Engine).

- **Session end — Notification engine root cause diagnosed and refactor plan saved (2026-07-08):** Live tests confirmed Start re-fires on resume, milestones/almost repeat, and Complete often missing. Root cause identified as corrupted engine state from mixed responsibilities: `START_TIMER` resets trackers on every call, `UPDATE_TIMER_STATE` overwrites trackers every 5s, and Complete only fires on transition. New lego-block architecture designed with Active Session Token, pure Notification Engine, Timing Engine owning trackers, Notifier Service, and Service Worker orchestration. Alarm strategy changed from short periodic alarms to per-stage absolute alarms to avoid MV3 throttling. Full plan saved to `.claude/memory/notification-engine-refactor-plan.md`; implementation pending next chat.

- **Session end — Notification Engine Rebuild complete, builds pending (2026-07-07):** Pieces 1–6 implemented and individually verified; full suite 747/751 passing with 4 pre-existing unrelated failures (`globals.test.ts` dark mode CSS ×2, `ProjectCard.target-edit.test.tsx` mock issue, `ProjectCard.delete.test.tsx` dialog issue); `npm run build` and `npm run build:ext` not yet run after full suite due to chained failure; live browser verification not started.

- **Notification Engine Rebuild Piece 6 — TimerPanel completed status for all focus scenarios (2026-07-07):** `components/timer/TimerPanel.tsx` now derives `isCompleted` for sub-piece mode from `activeSubPiece.status === "completed"` or `subPieceRemaining === 0`, and for project-only mode from target time reached; completed state disables Start/Reset and shows the existing TimerDisplay completed badge; `components/timer/__tests__/TimerPanel.session-summary.test.tsx` added 2 tests asserting Start/Reset are disabled and the completed indicator is visible after dismissing sub-piece and project-target summaries; `npx tsc --noEmit`, `npx vitest run components/timer/__tests__/TimerPanel.test.tsx` (27/27), `npx vitest run components/timer/__tests__/TimerPanel.session-summary.test.tsx` (9/9), and `npm run build` all pass.

- **Notification Engine Rebuild Piece 5 — focusSync + messageHandler baselines complete (2026-07-07):** `extension/lib/focusSync.ts` now sends `UPDATE_TIMER_STATE` with full session state including `projectElapsedBaseline`/`subPieceRemainingBaseline`, and `UPDATE_SCHEDULE_INPUTS` no longer omits baselines when present; `extension/lib/messageHandler.ts` `UPDATE_SCHEDULE_INPUTS` handler explicitly merges stored state so baselines and trackers (`startFired`, `lastMilestone`, `almostDoneFired`, `completeFired`) survive `allocatedMinutes`/`targetTimeSeconds` patches; `focusSync.test.ts` +5 tests and `controlMessage.test.ts` +1 test; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/focusSync.test.ts` (18/18), `npx vitest run extension/lib/__tests__/controlMessage.test.ts` (11/11), and `npm run build:ext` all pass.

- **Notification Engine Rebuild Piece 4b — timerEngine session-relative stage evaluation complete (2026-07-07):** `extension/lib/timerEngine.ts` now computes `sessionDuration`/`sessionElapsed` from session baselines and uses them for milestone/almost-done checks via `state.lastMilestone`/`state.almostDoneFired`; legacy `getLastMilestone`/`setLastMilestone`/`clearLastMilestone` storage helpers removed; completion logic unchanged; `startSession` records baselines and uses `state.startFired`; `extension/lib/__tests__/timerEngine.test.ts` added re-focus sub-piece test, updated legacy milestone assertions, and adjusted schedule-dependent expectations; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts` (71/71), and `npm run build:ext` all pass.

- **Notification Engine Rebuild Piece 4a — timerEngine trackers + baselines complete (2026-07-07):** `extension/lib/timerEngine.ts` now clears notification tracker fields on real `startSession` and `resetSession`, preserves them across `pauseSession`, `startSession` resume, `updateScheduleInputs`, and `restoreOnStartup`, and preserves `projectElapsedBaseline`/`subPieceRemainingBaseline` across schedule-input updates; `extension/lib/__tests__/timerEngine.test.ts` added 5 tracker/baseline tests and updated schedule-dependent expectations; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts` (70/70), and `npm run build:ext` all pass.

- **Notification Engine Rebuild Piece 3 — stageScheduler percentage-based schedule complete (2026-07-07):** `extension/lib/stageScheduler.ts` now exposes percentage-based `startTime`, `milestoneTimes`, `almostTime`, and `completeTime`, with `getMilestoneCount` buckets up to 12 milestones and milestones evenly distributed inside the 25%-75% phase; legacy `milestones`/`almostDone`/`complete` aliases kept for existing consumers; `extension/lib/__tests__/stageScheduler.test.ts` rewritten with 9 tests covering 1-minute, 5-minute, 90s, 10-minute, 1-hour, 5-hour, phase invariants, monotonicity, and invalid input; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/stageScheduler.test.ts`, and `npm run build:ext` all pass.

- **Notification Engine Rebuild Piece 2 — useTimer baselines complete (2026-07-07):** `hooks/useTimer.ts` now computes, persists, restores, and sends `projectElapsedBaseline` and `subPieceRemainingBaseline` in `buildExtensionState`; baselines are set on real Start/Reset/re-focus, preserved across Pause/Resume via a `wasStarted` ref, and stored in `ff_active_session`; `hooks/__tests__/useTimer.test.tsx` added 3 baseline tests; `npx tsc --noEmit`, `npx vitest run hooks/__tests__/useTimer.test.tsx` (49/49), and `npm run build:ext` all pass.

- **Notification Engine Rebuild Piece 1 — Types complete (2026-07-07):** `ExtensionTimerState` extended with optional `projectElapsedBaseline`, `subPieceRemainingBaseline`, `startFired`, `lastMilestone`, `almostDoneFired`, and `completeFired`; `extension/lib/__tests__/types.test.ts` created with full-state JSON round-trip test; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/types.test.ts`, and `npm run build:ext` all pass.

- **Notification Fix Plan Piece 3 complete (2026-07-07):** `hooks/useTimer.ts` now sends `PAUSE_TIMER` via `sendTimerCommand` before clearing `ff_active_session` in both the sub-piece auto-complete and project target-crossed branches; `hooks/__tests__/useTimer.test.tsx` added 2 tests asserting `PAUSE_TIMER` is dispatched on sub-piece completion and on project target reach; `npx tsc --noEmit` and `npx vitest run hooks/__tests__/useTimer.test.tsx` (46/46) pass.

- **Notification Fix Plan Piece 2b complete (2026-07-07):** `extension/lib/messageHandler.ts` now handles `UPDATE_SCHEDULE_INPUTS` with payload validation; `extension/lib/timerEngine.ts` added `updateScheduleInputs` that patches `allocatedMinutes`/`targetTimeSeconds` while preserving runtime state and broadcasting `STATE_UPDATED`; `controlMessage.test.ts` +4 tests and `timerEngine.test.ts` +4 tests passing; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/controlMessage.test.ts`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts`, and `npm run build:ext` all pass.

- **Report screenshots fix (2026-07-05):** Added screenshots to the existing `report.md` at the repo root (not a new file). Removed the mistakenly created `docs/PROJECT_REPORT.md`. Screenshot paths use `docs/screenshots/*.png` from the root-level report. Committed and pushed to `main`.
...
- **Notification Engine Refactor Phase 5a — useTimer builds token and sends extension commands complete (2026-07-08):** `hooks/useTimer.ts` now builds `ActiveSessionToken` via `buildActiveSessionToken()` and sends `START_SESSION`/`RESUME_SESSION`/`PAUSE_SESSION`/`RESET_SESSION` via `window.browser.runtime.sendMessage` when available; `restart()` also sends `START_SESSION` with `isRunning: false`; legacy `sendTimerCommand`/`buildExtensionState` removed; `ff_active_session` localStorage persistence kept as fallback; `hooks/__tests__/useTimer.test.tsx` mocks `runtime.sendMessage` and asserts command types, token fields, and baseline preservation; `npx tsc --noEmit`, `npx vitest run hooks/__tests__/useTimer.test.tsx` (49/49), and `npm run build` all pass. Ready for Phase 5b (TimerPanel wiring and authoritative state reads).
...
- **Notification Fix Plan Piece 4 complete (2026-07-07):** Added end-to-end invariant test `5-min sub-piece with 30s alarm ticks: Start → M1 → M2 → M3 → Almost Done → Complete` to `extension/lib/__tests__/timerEngine.test.ts`; asserts `projectElapsed === 300`, `subPieceRemaining === 0`, `isRunning === false`, and exact notification stage sequence; `npx tsc --noEmit`, `npx vitest run extension/lib/__tests__/timerEngine.test.ts` (65/65), and `npm run build:ext` all pass.
...
- **Piece 1 — Remove TimerToast complete (2026-07-07):** Deleted `components/timer/TimerToast.tsx` and `components/timer/__tests__/TimerToast.test.tsx`; removed all `TimerToast` wiring from `components/timer/TimerPanel.tsx` (imports, `toastTrigger` state, `prevIsRunningRef`/`lastMilestoneRef`/`lastTierRef` refs, start/milestone/tier-change effects, `handleToastShown` helper, JSX nodes); kept `motivationContext`, `getMotivation`, `playCompleteSound`, and `playMilestoneSound` imports; completion summary, dialogs, controls, extension listeners, and budget warnings unchanged; `npx tsc --noEmit` clean; `TimerPanel.test.tsx` 27/27, `TimerPanel.session-summary.test.tsx` 7/7, `TimerDisplay.test.tsx` 14/14, `TimerControls.test.tsx` 6/6 passing.

- **Notification Fix Plan Piece 2a — Web app sync schedule inputs to extension (2026-07-07):** `hooks/useTimer.ts` `SessionData` interface gained `allocatedMinutes?: number`; `persistSession` now includes `allocatedMinutes: subPiece?.allocatedMinutes` and updates its dependency array; `extension/lib/focusSync.ts` re-implemented `syncFocusSession()` to read `ff_active_session`, validate `projectId`/`allocatedMinutes`/`targetTimeSeconds` types, and send `browser.runtime.sendMessage({ action: "UPDATE_SCHEDULE_INPUTS", payload: { projectId, subPieceId, allocatedMinutes, targetTimeSeconds } })`; errors from missing extension context are swallowed; `startFocusSyncPolling` now calls `syncFocusSession` at start and inside each interval alongside `syncExtensionSettings`; `hooks/__tests__/useTimer.test.tsx` added persist test (44/44 passing); `extension/lib/__tests__/focusSync.test.ts` replaced old syncFocusSession tests with 6 tests covering valid payload, omitted optional fields, invalid context, missing/malformed session, and missing `projectId` (13/13 passing); `npx tsc --noEmit` clean; `npm run build:ext` succeeds.

- **Notification Fix Plan Piece 2b — Extension handle schedule input updates (2026-07-07):** Added `UPDATE_SCHEDULE_INPUTS` to `TimerMessage` union in `extension/lib/messageHandler.ts`; added handler validating `projectId` and numeric `allocatedMinutes`/`targetTimeSeconds`, routing to `timerEngine.updateScheduleInputs`; added `updateScheduleInputs` in `extension/lib/timerEngine.ts` under `withLock`, reading stored state, verifying matching `projectId`, patching only valid `allocatedMinutes`/`targetTimeSeconds` while preserving `projectElapsed`/`subPieceRemaining`/`isRunning`/`savedAt`/names/IDs, saving via `setTimerState`, and broadcasting `STATE_UPDATED` without recreating alarms or touching notification trackers; `controlMessage.test.ts` added 2 tests (10/10 passing); `timerEngine.test.ts` added 4 tests (64/64 passing); `npx tsc --noEmit` clean; `npm run build:ext` succeeds.

- **Notification Fix Plan Piece 3 — Pause extension on web-app completion (2026-07-07):** Added `void sendTimerCommand("PAUSE_TIMER")` in `hooks/useTimer.ts` RAF `tick` loop in both the sub-piece auto-complete branch and the project target-crossed branch, placed before `localStorage.removeItem(SESSION_KEY)` and `onCompleteRef.current?.()`; `hooks/__tests__/useTimer.test.tsx` added 2 tests asserting `PAUSE_TIMER` is dispatched on sub-piece completion and project target completion; `npx tsc --noEmit` clean; `hooks/__tests__/useTimer.test.tsx` 46/46 passing.

- **Notification Fix Plan Piece 4 — End-to-end invariant test (2026-07-07):** Added `"5-min sub-piece with 30s alarm ticks: Start → M1 → M2 → M3 → Almost Done → Complete"` test to `extension/lib/__tests__/timerEngine.test.ts` inside the existing `notification state-machine invariants` suite; test starts a 5-minute sub-piece, simulates ten 30-second alarm ticks via `setDrift(30)` + `tick()`, asserts `projectElapsed === 300`, `subPieceRemaining === 0`, `isRunning === false`, exact notification sequence `["start", "milestone", "milestone", "milestone", "almostDone", "subPieceComplete"]`, and each stage fires exactly once; `npx tsc --noEmit` clean; `extension/lib/__tests__/timerEngine.test.ts` 65/65 passing; `npm run build:ext` succeeds.

- **Notification engine rebuild spec agreed (2026-07-07):** User confirmed the notification engine must be rebuilt with a unified percentage-based pattern: Start at 0%, milestones evenly distributed inside 25%–75%, Almost Done at 82.5% (midpoint of 75%–90%), Complete at 100%; milestone count scales with duration (1–2 min → 1, >2–5 min → 2, >5–10 min → 3, >10–20 min → 4, >20–40 min → 5, >40–60 min → 6, >1–2 hr → 8, >2–4 hr → 10, >4 hr → 12); session-relative progress requires adding `projectElapsedBaseline` and `subPieceRemainingBaseline` to `ExtensionTimerState`; `TimerPanel.tsx` `isCompleted` derivation must be fixed so completed status and disabled Start/Reset buttons appear when user clicks X on the summary dialog; full spec saved to `.claude/memory/notification-engine-rebuild-spec.md` and indexed in `MEMORY.md`; implementation pending Plan Mode in next chat.

2026-07-08 — Phase 1 Active Session Token + sessionStorage wrapper implemented; types, storage wrapper, and 3 tests pass; tsc + vitest + build:ext green.

- 2026-07-08: Phase 3b complete — timerEngine per-stage absolute alarms + notification firing; 16 tests passing.- 2026-07-08 Cleanup A: renamed elapsedPausedSeconds -> elapsedActiveSeconds across extension types, timerEngine, messageHandler, and all related tests; tsc + tests + build:ext pass.
- 2026-07-09: Phase 6 complete — removed old sync remnants from `extension/entrypoints/control.content.ts` (dropped EXT_* forwarding and ff:command/ff:request listeners, kept STATE_UPDATED → ff:state), updated `extension/lib/__tests__/control.content.test.ts` (2 tests), removed legacy tracker fields from `ExtensionTimerState` and updated `extension/lib/__tests__/types.test.ts`, clarified `ff_active_session` in `hooks/useTimer.ts` as web-app restore only; verification: tsc clean, control.content 2/2, useTimer 49/49, TimerPanel 30/30, `npm run build` and `npm run build:ext` both pass.

2026-07-09 — Phase 7 integration tests added: 4 end-to-end focus-session tests pass, including 1-min, 5-min, pause/resume, and reset scenarios.

- **Notification Engine Synchronization v2 Piece 5 — Complete at 100% + Restore on Startup complete (2026-07-10):** `restoreOnStartup()` now clears stale focus alarms before recalculating; added complete-too-early guard test to `notificationEngine.test.ts` and 6 restore-on-startup tests to `timerEngine.test.ts` (no session, paused session, below-target drift, target-reached-while-closed, >60-min cap, already-fired dedup); `npx tsc --noEmit` clean; `npx vitest run extension/lib/__tests__/notificationEngine.test.ts` (27/27) and `extension/lib/__tests__/timerEngine.test.ts` (23/23) pass. Ready for Piece 6.

- **Notification Engine Synchronization v2 Piece 6 — Acceptance scenario + re-focus tests complete (2026-07-10):** Added integration tests to `extension/lib/__tests__/integration.test.ts` covering Start→Pause→Resume→Complete, Reset while paused, old-session alarm rejection after reset/new start, extension restart restore/recalculation, and re-focus remaining duration for both sub-piece (`subPieceRemainingBaseline`) and project (`projectElapsedBaseline`) modes; `npx tsc --noEmit` clean; targeted extension tests pass (`notificationEngine` 27/27, `timerEngine` 23/23, `integration` 10/10); `npm run build:ext` succeeds. Notification Engine Synchronization v2 complete.

- **Extension popup bugfix — active session display + read-only popup (2026-07-10):** Fixed popup fallback to read the authoritative `ff_active_session_v2` key after `GET_ACTIVE_SESSION` message misses; removed dead `ff_extension_timer` fallback; removed Start/Pause/Reset buttons from `popup.html` and `popup.ts` to prevent a second control surface from desynchronizing the web-app timer and extension notifications; updated `popup.test.ts` (17/17) and deleted obsolete `popup-start.test.ts` and `popup-pause-reset.test.ts`; `npx tsc --noEmit` clean, all extension lib tests pass (202/202), `npm run build:ext` succeeds.

- **Extension popup polish — trimmed times + completed status (2026-07-10):** `formatDuration` now floors fractional seconds so popup times no longer show long floating-point artifacts; popup now shows a "ပြီးစီး (Completed)" status with a green dot when a sub-piece has no remaining time or a project has reached its target; added `lib/__tests__/time.test.ts` (6 tests) and 2 completed-status tests to `popup.test.ts`; `npx tsc --noEmit` clean, extension + time tests pass (208/208), `npm run build:ext` succeeds.

- **Extension popup live-drift fix (2026-07-10):** `tokenToDisplayState()` in `extension/lib/popup.ts` now computes live drift (`Date.now() - resumedAt`) for running sessions so the popup stays in sync with the web-app TimerPanel instead of lagging behind the 5-second storage persist interval; paused sessions still use the last persisted `elapsedActiveSeconds`; added live-drift tests to `popup.test.ts`; `npx tsc --noEmit` clean, extension + time tests pass (210/210), `npm run build:ext` succeeds.

- **Extension popup scope fix (2026-07-10):** Popup elapsed/remaining cards now match the active focus mode: sub-piece focus shows sub-piece elapsed/remaining, project-only focus shows project elapsed/remaining; labels update dynamically; completion status still derived from the same live values; updated `popup.test.ts` (26 tests); `npx tsc --noEmit` clean, extension + time tests pass (211/211), `npm run build:ext` succeeds.

- **Session save + branch created (2026-07-10):** Current working tree committed as `433c338` and pushed to `origin main`; new branch `fix/popup-web-app-sync` created from `main`; next piece is Option B (popup mirrors web-app state via `ff_display_state`); resume point saved to `.claude/memory/popup-web-app-sync-resume.md`.
